...
/// ★ 받은 파일 항목 붙여넣기 방식(09-12 · docs/08 §3-1) — 첫 후보가 기본(적응형 = 안 "다").
const SYNC_FILES_OPTS: &[(&str, Msg)] = &[
    ("adaptive", Msg::SyncFilesAdaptive),
    ("text", Msg::SyncFilesTextOnly),
];
...
pub(crate) const REGISTRY: &[Entry] = &[
    ...
    // ★ 파일 경로 전파(09-12 · DR-6 "파일은 내용이 아니라 목록" · docs/08 §3) —
    //   보내는 쪽 스위치·상한과 받는 쪽 붙여넣기 방식이 한 묶음으로 붙는다.
    e(
        Msg::CatSync,
        Msg::SetSyncFiles,
    ...
    e(
        ...
        SettingKind::Radio(SYNC_FILES_OPTS),
        "sync.files_paste",
    ),
    e(
        Msg::CatSync,
