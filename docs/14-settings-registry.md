...
| `ui.set_x` · `ui.set_y` · `ui.set_w` · `ui.set_h` · `ui.set_mon` | 숨김 | — | ★ **설정 창 마지막 기하 + 모니터 키**(09-04 사용자) — 닫을 때 저장 · 열 때 **메인창이 같은 모니터에 있을 때만** 복원, 아니면 메인창 옆(오른쪽 → 왼쪽 → 겹침)으로 초기화 · 메인창 최상위면 설정 창도 최상위 | N |
...
| key | 종류 | 기본값 | 라벨 | 출처 |
|---|---|---|---|:--:|
| `find.mode` | Radio | **유사**(09-04 사용자 확정 · 종전 정확히) | ★ **검색 방식**(정확히/유사/정규식) — [§2-6](#2-6--검색-모드를-설정으로-둔다). ★ 09-04 실구현: **라벨 + 본문 전체** · 대소문자 무시 · 정확히 = 부분 문자열 · 유사 = 띄어쓴 단어 전부(순서 무관) · 정규식 = 자체 엔진 `nclip_core::search`(. * + ? {n,m} [] () \| ^ $ \d \w \s \b · 단계 상한 20만 · 오류 = 정확히 폴백) · 설정 변경은 다음 박동에 열린 창 재필터 | M |
...
| `sync.device_name` | Text | — | 기기 표시 이름(비면 호스트명 정제 → `clip-{지문4}` 폴백 · 프로필 실행은 `-{프로필}` 접미) · ★ 변경 1s 뒤 연결된 세션 전부(릴레이·LAN · 승인 무관)에 재소개 — 상대는 PeerId 기준 자동 갱신(09-05) |
...
| ★ `sync.file_contents` | Switch | **on** | **파일 내용 공유**(09-12 · DR-30/43) — 다른 기기의 파일 항목을 붙여넣을 때 그 기기에서 받아 **진짜 파일로**(로컬 캐시에 보관·재사용) · 끄면 경로만 · 보내는 쪽도 서빙 거부 |
