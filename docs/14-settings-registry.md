...
| key | 종류 | 기본값 | 라벨 | 출처 |
|---|---|---|---|:--:|
| `find.mode` | Radio | **정확히** | ★ **검색 방식**(정확히/유사/정규식) — [§2-6](#2-6--검색-모드를-설정으로-둔다). ★ 09-04 실구현: **라벨 + 본문 전체** · 대소문자 무시 · 정확히 = 부분 문자열 · 유사 = 띄어쓴 단어 전부(순서 무관) · 정규식 = 자체 엔진 `nclip_core::search`(. * + ? {n,m} [] () \| ^ $ \d \w \s \b · 단계 상한 20만 · 오류 = 정확히 폴백) · 설정 변경은 다음 박동에 열린 창 재필터 | M |
