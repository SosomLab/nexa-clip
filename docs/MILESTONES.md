...
| ✅ | ★ **창 안 단축키 14종 설정 승격 + 스택 순차 붙여넣기 4변형(09-08 · T-48)** — 설정 → 단축키에 **전역/창 안** 그룹 · `WINDOW_ACTIONS`(선택/평문/N번째/스택 담기/순차 원본·평문 × 줄바꿈/고정/삭제/설정/보기 3) · 판정 `keys::Keymap`(물리 키 자리 · 수식 키 정확 일치) · 배지/상태줄/툴팁/푸터 = 설정값 · 줄바꿈 = `send_newline` Enter 주입(3-OS). 09-07 키맵 정비(T-47 `Ctrl+1~9`·`Alt+1/2/3`·`Ctrl+,`)의 완결. **실기 ⏳** [21 §12-1](21-manual-test.md) |
