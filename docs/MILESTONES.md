...
| ✅ | ★ **K-1 키 주입**(Win 실사용 통과 · **복원 경로는 미검증** · mac **빌드 복구됨** · 실기 미검증) · `nclip-plat::{font,paths}` 이식 |
...
| 🚧 | **LAN 직결 동기화** ← 사용자 실익이 가장 큰 지점 — **TCP 직결 1단 ✅(09-04)** · ★ **두 PC 세션 성립 ✅(09-05 · mac 호스트 ↔ Linux VM)** · 릴레이 None 자동 재기동 ✅ · ⚠️ 인터페이스 열거(가상 어댑터 · 비콘 한 방향 실증) ☐ · 승인→전파 실기 ⏳ |
