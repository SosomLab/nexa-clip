...
| 🚧 | 자동 전파(에코 차단 · 전순서 · 덮어쓰기 정책) — 평문·PNG 1단 ✅(09-04) · ★ **파일 경로 전파 ✅(09-12)** · ★ **파일 내용 공유 🔶(09-12 — DR-30/43 · 붙여넣을 때 받기 · 캐시 · 저속 사전 캐시 · 이어 받기 · 패널/트레이 · 525 테스트)** → ★ **두 기기 실기 1차 통과 ✅(09-13 · mac ↔ Linux VM)** — 약속 등재 → 사전 캐시 → 캐시 실체화 → **붙여넣기 SHA-256 동일**(EXIF 온전) · 캐시 게시 에코 `Promoted`(연쇄 0) · 09-13 후속 = 최신 항목 즉시 받기 + 맨 앞이면 자동 게시 · 붙여넣기 모드 3결함 수정(평문 무시 · 경로 폴백 Windows 전용 · **X11 합성 텍스트 별칭 누락**) · 543 테스트 · ⏳ 잔여 실기 S13·S16 후반·S17·S19~S21(GUI 조작) · 리치·덮어쓰기 정책·되돌리기 ☐ |
| ☐ | 신뢰 기기와 **공유**(수동·보드 단위) |
