...
| ✅ | ★ **v0.1.3(09-08)** — 스택 순차 4변형 · 창 안 단축키 설정 · mac 권한 유령 항목 정리 · ★ **winget 첫 제출**(수동 검수 ⏳) · choco 보류(T-50) · brew 자동 |
...
| ✅ | ★ **사용자 설명서 = 위키 10페이지 + 화면 캡처 20장(09-05)** — 설치(권한·데이터 위치·업그레이드) · 시작하기(단축키·팝업·메인창 보기 3모드·검색) · 설정 항목(10 카테고리 화면) · 동기화/LAN/중복 제외/프로필 · 문제 해결(OS별) · ★ **릴레이 서버**(beepd 별도 실행 · nexa-beep 배포). 캡처 = `--profile docs` 자동 촬영 · 저장소 `assets/screenshots/` 사본. 다음 = 홈페이지(T-43) |
| ✅ | ★ **창 안 단축키 14종 설정 승격 + 스택 순차 붙여넣기 4변형(09-08 · T-49)** — 설정 → 단축키에 **전역/창 안** 그룹 · `WINDOW_ACTIONS`(선택/평문/N번째/스택 담기/순차 원본·평문 × 줄바꿈/고정/삭제/설정/보기 3) · 판정 `keys::Keymap`(물리 키 자리 · 수식 키 정확 일치) · 배지/상태줄/툴팁/푸터 = 설정값 · 줄바꿈 = `send_newline` Enter 주입(3-OS). 09-07 키맵 정비(T-47 `Ctrl+1~9`·`Alt+1/2/3`·`Ctrl+,`)의 완결. **실기 ⏳** [21 §12-1](21-manual-test.md) |
