...
>
> **상태**: 🔶 ★ **구현됨(09-12 · T-31c · [journal](journal/2026-09-12.md))** — 단 ★ **DR-43 캐시 우선**으로: §2의 `IDataObject`/파일 약속(지연 렌더링) 대신 **붙여넣기 시점에 받아 로컬 캐시에 실체화한 뒤 `CF_HDROP`(등)로 게시**한다(3-OS 같은 길 · D-64의 Linux 방식을 전 OS로). 규약은 **당겨 받기(pull)** `FileReq{offset,len}` → `FileData`×n + `FileEnd{size,mtime}`. 이어 받기(`.part`) · 저속 사전 캐시(Bg) · 우선순위(Fg) · 진행 패널 · 트레이 · 설정 5키 — §6 설정 표 참조. `IDataObject` 스트리밍은 필요해질 때 캐시 위에 얹는다.
> 경로 목록 전파([DR-6](10-decision-record.md) · [08 §3](08-clipboard-propagation.md))는 그대로 살아 있고, 약속은 그 위에 얹힌다(경로가 실재하면 네트워크를 타지 않는다 — P-3).
>
...
| 키 | 기본 | 무엇 |
|---|---|---|
| `sync.file_contents` | 켬(내 기기 한정) | 파일 내용 공유 자체 — ✅ 09-12 |
| ★ `sync.file_auto_mb`(설계명 `file_precache_mb`) | **50**(D-65의 1MB에서 상향 — 사용자 09-12 "이하 용량이면 백그라운드로 천천히 캐싱") | 항목 **합계**가 이하면 받자마자 저속 사전 캐시 · 0 = 끔 — ✅ |
| ★ `sync.file_bg_kbps` | 1024 · 0 = 무제한 | 백그라운드 속도 상한(Fg는 제한 없음) — ✅ 09-12 추가 |
| ★ `sync.file_max_mb` | **1000**(D-67의 100에서 상향 — 붙여넣기 대기 전송은 사용자가 지금 원하는 것) | 붙여넣기 시 받아올 **합계** 상한(D-68) — 넘으면 경로만 + 로그 — ✅ |
| ★ `sync.file_cache_mb` | 2000 | 캐시 용량 — 초과 시 오래된 디렉터리부터(전송 중 제외) — ✅ 09-12 추가 |
