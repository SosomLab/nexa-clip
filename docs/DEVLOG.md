...
- `packaged_install()` 판정 추가 · `install_launcher`는 사용자 사본을 지우고 손 뗌 · 자동 실행은 패키지 실행 파일을 가리킴 · 포터블은 종전 동작 · 개발 빌드 실행으로 재현 시도 → 하이재킹 없음 확인 → [journal 11차](journal/2026-09-05.md)
