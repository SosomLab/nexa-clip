...
## 2026-09-13 (6차 · Linux VM) — ★ **파일 항목 붙여넣기 3결함 수정** — ⓐ 평문이 `원본` 가지로 흘러 무시됨 · ⓑ 경로 합성 폴백이 `CF_HDROP` 전용(Linux·mac 무반응) · ★ ⓒ **X11 게시가 합성 텍스트에 `UTF8_STRING`·`STRING` 별칭을 안 달았다**(= "게시·주입은 성공인데 안 붙는" 진범 · 09-05 감사 T-41 ③ 해소) · 538 테스트(신규 5 + 실 X11 왕복) · 3타깃+fmt ✓
...
## 2026-09-13 (5차 · Linux VM) — 저장소 최신화(clip 32 ff · v0.1.2/v0.1.3 · beep 66 ff · v0.2.14/v0.2.15) · Linux 게이트 **533 테스트 ✓ · 3타깃 clippy ✓** · 설치본 **0.1.1 → 0.1.3** 교체·재시작(mac과 LAN 직결 즉시 성립) · 원장 **I-11**(beep R-22 탐지 가능성 → clip 판 = T-54)
...
## 2026-09-13 (1차 · mac) — ★ 전송 패널 토글 버튼(툴바 바닥 · 연결/⚙ 위 · 자동 열림/닫힘 + 진행 점) · 설정 설명 단위 표기(Number 9항목 × 4언어) · ★ 받은 파일 저장 폴더 `sync.file_dir`(기본 = OS 다운로드 폴더/`Nexa Clip` · Linux XDG · 옛 폴더도 캐시 판정 유지) · 536 테스트 · 3-OS ✓ → [journal](journal/2026-09-13.md)

## 2026-09-12 (6차 · win) — ★ 정보(About) 화면 — 설정 → 정보: 버전 · git SHA(+dirty)·빌드 시각(build.rs) · 실행 파일 경로·크기·**SHA-256**(부팅 워커) · OS · 데이터 폴더 · [복사] · i18n 13×4 · 530 테스트 · 3-OS ✓ · 설치본 교체 → [journal](journal/2026-09-12.md)

## 2026-09-12 (5차 · win) — 왜 텍스트 가드·검증이 파일에서 무력했나(답: 가드 셋·검증 하나가 전부 "게시 = 수신" 전제 · 캐시 게시는 설계상 ≠ · 가드 층엔 테스트 없고 테스트 층엔 가드 없음) · `from_reps` 캐시 경로 페이로드 금지 + 단위 테스트 · 527 ✓ → [journal](journal/2026-09-12.md)

## 2026-09-12 (4차 · win) — ★ 2PC 연쇄 반응(핑퐁) 수정 — 사전 캐시 완료는 게시 안 함 · 캐시 게시 에코는 약속 항목 승격으로 흡수(`History::promote`) · 캐시 경로 파일 항목 전파 금지(이중) · 526 테스트 · 3-OS ✓ · 설치본 재교체

- 고리 = 캐시 실체화 후 `CF_HDROP` 게시 → 감시 되읽음 → 새 로컬 항목 → 상대 전파 → 상대 캐시·게시 → … (사전 캐시 완료 게시로 조작 없이 자동) · 붙여넣기 이름 = 원본 base name 확인 → [journal](journal/2026-09-12.md)
...
## 2026-09-07 (3차 · mac) — 저장소 최신화(clip 2 ff · beep 5 ff · 와이어 0) · ★ brew 업그레이드 뒤 손쉬운 사용 권한 유령 항목 자동 정리(앱 `warm_up` + Cask postflight `tccutil reset`) · T-48 서명 신원 결정 등재

- 원인 = 애드혹 서명(TCC 요구사항 = `cdhash`)이라 업그레이드마다 옛 항목 무효(껐다 켜도 안 됨 · 삭제만 통함 — 사용자 실기) → 앱 시작·Cask 설치 시 자기 번들 ID로 리셋 후 대화상자 → 사용자는 [켜기]만 · 근본 = 안정 서명 신원(T-48 🔴) · 3-OS clippy ✓ · ⏳ 다음 릴리스 실기 → [journal](journal/2026-09-07.md)
...
## 2026-09-05 (11차) — ★ 아이콘이 개발 빌드를 띄우던 것 근절 — 패키지 설치본이 있으면 사용자 런처·자동 실행을 건드리지 않는다

- `packaged_install()` 판정 추가 · `install_launcher`는 사용자 사본을 **지우지 않고 설치본을 가리키게 유지**(1차의 "지운다"는 gnome-shell이 삭제된 경로를 캐시해 실패) · 자동 실행은 패키지 실행 파일을 가리킴 · 포터블은 종전 동작 · ★ **사용자 실기 ✓ — 단 이미 떠 있는 셸 캐시 때문에 재시작(리부팅)까지가 절차** → [journal 11차](journal/2026-09-05.md)

## 2026-09-05 (10차) — ★ 전역 단축키 불능 = GNOME 저장소에 VS Code 이름으로 등록된 우리 단축키 · 스코프 이름 `\x2d` 규칙

- 포털은 cgroup 유닛에서 앱 이름을 읽는다 → 에디터 터미널에서 띄우면 `code`로 등록돼 정상 실행분이 충돌로 실패 · `Err(_)`로 버리던 포털 오류를 로그에 남겨 근인 규명(`An app id is required`) · 스크립트를 앱 이름 스코프(`app-nexa\x2dclip-…`)로 · `.deb` 아이콘 등록은 정상이나 앱이 사용자 런처를 자기 경로로 덮어써 마지막 실행 빌드가 아이콘을 가져감 → [journal 10차](journal/2026-09-05.md)

## 2026-09-05 (마감) — ★ Linux 세션 총정리 — 설치본 기준 실기 체제 · 사용자 제보 3건 해결 · 다른 OS 파급 점검 · beep 전달문

- 커밋 14(전부 green) · 지금 도는 것 = `/usr/bin/nexa-clip` + `~/.config/nexa-clip`(이력 38 · mac LAN 세션) · 개발 빌드는 별도 신원으로 분리 · ⏳ 실기 5건 · ⏳ 결정 2건(beep 전달 · D-80) → [STATUS](STATUS.md)
